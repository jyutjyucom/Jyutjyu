  ...
  <div class="space-y-4 font-cjk-content">
    ...
    <template v-if="isTextSearch && displayedGroupedResults.exactMatches.length > 0">
      <div
        ...
        <svg class="w-4 h-4 text-archive-green dark:text-archive-green-light" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <span class="text-archive-green dark:text-archive-green-light text-sm font-semibold">
      ...
      <div class="md:hidden space-y-2">
        ...
        >
          <div class="flex items-baseline gap-2 mb-1">
            <span class="text-lg font-sung-content font-semibold text-ink dark:text-parchment">{{ group.primary.headword.display }}</span>
          ...
          <tbody class="bg-surface-low dark:bg-stone-900 divide-y divide-outline-soft/20 dark:divide-stone-800">
            <template v-for="group in displayedGroupedResults.exactMatches" :key="group.key">
              <tr class="hover:bg-surface-high dark:hover:bg-stone-800 cursor-pointer transition-colors" role="link" tabindex="0" @click="openWordPage(group)" @keydown.enter.prevent="openWordPage(group)">
                <td class="px-3 whitespace-nowrap py-2"><NuxtLink :to="getWordPath(group)" class="text-lg font-sung-content font-semibold text-ink dark:text-parchment hover:text-kapok transition-colors" @click.stop>{{ group.primary.headword.display }}</NuxtLink></td>
