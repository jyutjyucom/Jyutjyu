<template>
  <div class="space-y-4 font-cjk-content">
    <!-- 完全匹配的结果（仅文字搜索时显示） -->
    <template
      v-if="isTextSearch && displayedGroupedResults.exactMatches.length > 0"
    >
      <div
      ...
      >
        <svg
          class="w-4 h-4 text-archive-green dark:text-archive-green-light"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        >
          ...
          <div class="flex flex-wrap gap-1 mt-1">
            <span
              ...
              :key="source"
              class="px-1.5 py-0.5 text-xs bg-kapok/10 dark:bg-kapok/20 text-kapok"
              >{{ source }}</span
            >
          </div>
        </div>
      ...
      <!-- Desktop: table -->
      <div
        class="hidden md:block bg-surface-low dark:bg-stone-900 overflow-hidden"
      >
        <table class="w-full">
          <thead class="bg-surface-high dark:bg-stone-800">
            <tr>
              <th
                class="px-3 text-left text-base font-medium uppercase tracking-wider py-2 text-ink dark:text-parchment"
              >
                {{ t("common.wordColumn") }}
              </th>
              <th
          ...
          <tbody
            class="bg-surface-low dark:bg-stone-900 divide-y divide-outline-soft/20 dark:divide-stone-800"
          >
            <template
              v-for="group in displayedGroupedResults.exactMatches"
              :key="group.key"
              ...
              >
                <td class="px-3 whitespace-nowrap py-2">
                  <NuxtLink
                    ...
                    class="text-lg font-sung-content font-semibold text-ink dark:text-parchment hover:text-kapok transition-colors"
                    @click.stop
                ...
                <td class="px-3 py-2">
                  <div
                    class="text-base text-ink/80 dark:text-stone-300 line-clamp-2"
                  >
                    {{ getGroupDefinitions(group) || "-" }}
                  </div>
                </td>
                <td class="px-3 whitespace-nowrap py-2">
                  <div class="flex flex-wrap gap-1">
                    <span
                      v-for="source in getGroupSources(group)"
                      :key="source"
                      class="px-2 py-1 text-sm bg-kapok/10 dark:bg-kapok/20 text-kapok rounded-md"
                      >{{ source }}</span
                    >
                  </div>
                </td>
    ...
    <template v-if="displayedGroupedResults.otherResults.length > 0">
      ...
      >
        <svg
          class="w-4 h-4 text-muted-gold dark:text-amber-300"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        >
          ...
          <span
            class="ml-1 px-1.5 py-0.5 bg-muted-gold/10 dark:bg-amber-900/30 text-muted-gold dark:text-amber-300"
            >{{ groupedResults.otherResults.length }}</span
          >
          {{ t("common.remainingSuffix") }}
          ，{{
            t("dictCard.collectedBy", { count: otherResultsDictionaryCount })
      ...
      >
        <table class="w-full">
          <thead
            class="bg-surface-low dark:bg-stone-800 border-b border-outline-soft/20 dark:border-stone-700"
          >
            <tr>
              <th
                class="px-3 text-left text-base font-medium uppercase tracking-wider py-2 text-ink dark:text-parchment"
            ...
            >
              ...
              >
                <td class="px-3 whitespace-nowrap py-2">
                  <NuxtLink
                    :to="getWordPath(group)"
                    class="text-base font-semibold text-ink dark:text-parchment hover:text-kapok transition-colors"
                    @click.stop
                    >{{ group.primary.headword.display }}</NuxtLink
                  >
                </td>
