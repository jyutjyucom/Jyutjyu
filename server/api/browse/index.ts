import { getBrowsePage, isBrowseScopeSupported } from '../../utils/browse-index'
...
export default defineEventHandler(async (event) => {
  ...
  const page = Math.max(1, parseInt(getFirstQueryValue(query.page) || '1', 10) || 1)
  const size = Math.max(1, parseInt(getFirstQueryValue(query.size) || '100', 10) || 100)
