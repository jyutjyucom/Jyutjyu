...
const getDictionaryEntries = async (dict: DictionaryIndexItem): Promise<DictionaryEntry[]> => {
  ...
  if (dict.chunked && dict.chunk_dir) {
    const manifestRaw = await readFile(resolve(DICTIONARY_ROOT, dict.chunk_dir, 'manifest.json'), 'utf8')
...
const buildBrowseDatasetFromApi = async (): Promise<BrowseDataset> => {
        ...
        jyutpingSortKey: {
          $toLower: {
            $trim: {
              input: {
                $ifNull: [
                  { $arrayElemAt: ['$phonetic.jyutping', 0] },
                  ''
                ]
              }
            }
  ...
  ], { allowDiskUse: true }).toArray() as ApiCanonicalRow[]
  ...
  for (const row of rows) {
    ...
    const jyutpingSortKey = normalizeSpace(row.jyutpingSortKey || '').toLowerCase()
    ...
    if (jyutpingSortKey) {
      ...
      if (!existingJyutping || jyutpingSortKey.localeCompare(existingJyutping, 'en') < 0) {
    ...
    if (jyutpingSortKey) {
      ...
      if (!existingGlobalJyutping || jyutpingSortKey.localeCompare(existingGlobalJyutping, 'en') < 0) {
  ...
  const dictionaryScopes: BrowseDictionaryScope[] = dictionaries.map((dict) => {
    const headwordMap = headwordMapByDictionary.get(dict.id) || new Map<string, string>()
    const jyutpingMap = jyutpingMapByDictionary.get(dict.id) || new Map<string, string>()
    const scopeHeadwords = toScopeHeadwords(headwordMap, jyutpingMap)
    scopes.set(dict.id, scopeHeadwords)
    return {
      id: dict.id,
...
const getBrowseDataset = async (): Promise<BrowseDataset> => {
  ...
  browseDatasetPromise = buildBrowseDataset().then((dataset) => {
...
export const getBrowsePage = async (options: BrowsePageOptions): Promise<BrowsePageData> => {
  ...
  const sortedHeadwords = safeSort === 'jyutping'
