...
const loadEntriesFromFile = async (filePath: string): Promise<DictionaryEntry[]> => {
  const payload = await readJsonFile<unknown>(filePath)
  return Array.isArray(payload) ? payload as DictionaryEntry[] : []
}

const buildSuggestionRecords = async (): Promise<HeadwordSuggestionRecord[]> => {
  const index = await readJsonFile<DictionaryIndex>(DICTIONARY_INDEX_PATH)
  const dictionaries = Array.isArray(index?.dictionaries) ? index.dictionaries : []
  ...
  for (const dict of dictionaries) {
    if (dict.chunked && dict.chunk_dir) {
      ...
      for (const chunkFile of chunkFiles) {
        const entries = await loadEntriesFromFile(resolve(DICTIONARY_ROOT, dict.chunk_dir, chunkFile))
    ...
    const entries = await loadEntriesFromFile(resolve(DICTIONARY_ROOT, dict.file))
    entries.forEach((entry) => mergeSuggestionRecord(suggestions, entry))
  }
...
export const getHeadwordSuggestionRecords = async (): Promise<HeadwordSuggestionRecord[]> => {
