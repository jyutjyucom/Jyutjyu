...
const chunkedDictionaryEntriesPromiseCache = new Map<string, Promise<DictionaryEntry[]>>()
...
const toComparableKey = (value: string): string => normalizeSpace(value).toLowerCase()
...
const getDictionaryIndex = async (): Promise<DictionaryIndexItem[]> => {
  ...
  dictionaryIndexPromise = (async () => {
    ...
    const dictionaries = Array.isArray(parsed?.dictionaries) ? parsed.dictionaries : []
...
const readDictionaryEntriesFile = async (filePath: string): Promise<DictionaryEntry[]> => {
...
const getChunkEntries = async (chunkDir: string, chunkFile: string): Promise<DictionaryEntry[]> => {
...
const getAllEntriesForChunkedDictionary = async (dict: DictionaryIndexItem): Promise<DictionaryEntry[]> => {
...
const matchesQuery = (entry: DictionaryEntry, queryKeys: Set<string>): boolean => {
...
const findEntriesFromChunkedDictionary = async (
  ...
  for (const form of queryForms) {
    ...
    const variants = new Set([firstChar, firstChar.toLowerCase(), firstChar.toUpperCase()])
...
const resolveFromJson = async (headword: string): Promise<ResolvedWordResult | null> => {
  ...
  candidateEntries.push(...nonChunked.filter((entry) => matchesQuery(entry, queryKeys)))

  for (const dict of dictionaries) {
    if (!dict.chunked) continue
    const entries = await findEntriesFromChunkedDictionary(dict, queryForms, queryKeys)
    candidateEntries.push(...entries)
  }
...
const resolveFromApi = async (headword: string): Promise<ResolvedWordResult | null> => {
  const queryForms = await getQueryForms(headword)
  if (queryForms.length === 0) {
    return null
  }
...
const getCanonicalHeadwordsFromJson = async (): Promise<string[]> => {
  ...
  jsonCanonicalHeadwordsPromise = buildJsonCanonicalHeadwords().then((headwords) => {
...
const getCanonicalHeadwordsFromApi = async (): Promise<string[]> => {
  ...
  apiCanonicalHeadwordsPromise = (async () => {
    ...
    const rows = await collection.aggregate([
      {
        $project: {
          canonicalHeadword: {
            $let: {
              vars: {
                normalized: { $trim: { input: { $ifNull: ['$headword.normalized', ''] } } },
                display: { $trim: { input: { $ifNull: ['$headword.display', ''] } } }
              },
              in: {
                $cond: [
                  { $ne: ['$$normalized', ''] },
    ...
    ], { allowDiskUse: true }).toArray() as Array<{ canonicalHeadword: string }>
...
export const resolveWordEntries = async (headword: string): Promise<ResolvedWordResult | null> => {
  const cleaned = normalizeSpace(headword)
  if (!cleaned) {
    return null
  }
  ...
  if (useApi) {
    try {
      ...
      console.error('Word resolve (API mode) failed, fallback to JSON mode:', error)
...
export const getCanonicalHeadwords = async (): Promise<string[]> => {
  ...
  if (useApi) {
    try {
      ...
      console.error('Canonical headwords (API mode) failed, fallback to JSON mode:', error)
