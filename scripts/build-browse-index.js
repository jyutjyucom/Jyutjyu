...
const sortHeadwords = (headwords) =>
  [...headwords].sort((a, b) => a.localeCompare(b, "zh-Hant"));
...
const main = async () => {
  ...
  for (const [scopeId, scopeHeadwords] of dataset.scopes.entries()) {
    ...
    for (const pageSize of PAGE_SIZES) {
      for (const sort of SORTS) {
        ...
        const totalPages = await writePageFiles(
          scopeId,
          sort,
          pageSize,
          headwords,
  ...
  await writeFile(
    join(BROWSE_INDEX_ROOT, "manifest.json"),
    JSON.stringify(manifest, null, 2),
    "utf8",
  );
  const elapsedMs = Number(process.hrtime.bigint() - startTime) / 1_000_000;
  const totalScopes = Object.keys(manifest.scopes).length;
  const totalDictionaries = Array.isArray(manifest.dictionaries)
    ? manifest.dictionaries.length
    : 0;
