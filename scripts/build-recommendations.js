...
const SOURCE_FILES = [
  {
    ...
    file: "public/dictionaries/gz-colloquialisms.json",
  },
  {
    id: "gz-practical-classified",
    file: "public/dictionaries/gz-practical-classified.json",
  },
];
...
const toMinimalEntry = (entry) => {
  ...
  const jyutping = Array.isArray(entry?.phonetic?.jyutping)
    ? entry.phonetic.jyutping
    : [];
...
const isValidEntry = (entry) => {
  ...
  const definition = entry?.senses?.[0]?.definition || "";
  if (!definition || definition.includes("NO DATA") || definition.length < 3)
    return false;
...
console.log(
  `Recommendations generated: entries=${selected.length} total_candidates=${allEntries.length} elapsed_ms=${elapsed}`,
);
