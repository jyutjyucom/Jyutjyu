...
if (!subsetCommand) {
  if (existsSync(outputFontPath)) {
    console.warn('搵唔到 pyftsubset 或 python3 -m fontTools.subset，會沿用現有嘅 Chiron Hei UI 子集字型。')
    process.exit(0)
  }

  console.error('搵唔到 pyftsubset 或 python3 -m fontTools.subset，未能生成 Chiron Hei UI 子集字型。')
  process.exit(1)
}
