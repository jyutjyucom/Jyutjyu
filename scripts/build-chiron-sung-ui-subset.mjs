...
const outputItalicFontPath = resolve(
  outputDir,
  "chiron-sung-hk-ui-italic.woff2",
);
...
const hasExistingOutput =
  existsSync(outputFontPath) && existsSync(outputItalicFontPath);

if (!subsetCommand) {
  if (hasExistingOutput) {
    console.warn(
      "搵唔到 pyftsubset 或 python3 -m fontTools.subset，會沿用現有嘅 Chiron Sung UI 子集字型。",
    );
    process.exit(0);
  }

  console.error(
    "搵唔到 pyftsubset 或 python3 -m fontTools.subset，未能生成 Chiron Sung UI 子集字型。",
  );
  process.exit(1);
}
...
try {
  ...
  if (hasExistingOutput) {
    ...
    console.warn(
      `下載 Chiron Sung 原始字型失敗，會沿用現有嘅 UI 子集字型: ${message}`,
    );
    process.exit(0);
  }
