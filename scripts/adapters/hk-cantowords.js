...
export const DICTIONARY_INFO = {
  ...
  description: {
    "zh-Hans":
      "香港粤语社区词典，收录大量日常用语、俚语及现代词汇，提供香港粤语和英语双语释义",
    "zh-Hant":
      "香港粵語社區詞典，收錄大量日常用語、俚語及現代詞彙，提供香港粵語和英語雙語釋義",
    "yue-Hans":
      "香港粤语社区词典，收录大量日常用语、俚语同现代词汇，提供香港粤语同英语双语释义",
    "yue-Hant":
      "香港粵語社區詞典，收錄大量日常用語、俚語同現代詞彙，提供香港粵語同英語雙語釋義",
  },
  ...
  usage_restriction: {
    "zh-Hans":
      "此词典采用《非商业开放资料授权协议 1.0》，允许非商业使用、复制和修改。商业使用需获得授权（小型个人业务可豁免）。详见授权协议。",
    "zh-Hant":
      "此词典采用《非商业开放资料授权协议 1.0》，允许非商业使用、复制和修改。商业使用需获得授权（小型个人业务可豁免）。详见授权协议。",
    "yue-Hans":
      "此词典采用《非商业开放资料授权协议 1.0》，允许非商业使用、复制和修改。商业使用需获得授权（小型个人业务可豁免）。详见授权协议。",
  ...
  parts.forEach((part) => {
    ...
    if (colonIndex > 0) {
      ...
      if (headword && jyutpingStr) {
        ...
        const jyutpingParts = jyutpingStr
          .split(":")
          .map((p) => p.trim())
          .filter((p) => p);
...
export function transformRow(row) {
  ...
  const entry = {
    ...
    meta: {
      // 词头变体
      headword_variants:
        headwordVariants.length > 1
          ? headwordVariants.slice(1).map((v) => v.headword)
          : null,

      // 审核状态
      review_status:
        reviewStatus && reviewStatus.trim() ? reviewStatus.trim() : null,
      is_reviewed: reviewStatus && !reviewStatus.includes("UNREVIEWED"),

      // 公开状态
      publish_status:
        publishStatus && publishStatus.trim() ? publishStatus.trim() : null,
...
export function transformAll(rows) {
  ...
  processedRows.forEach((row, index) => {
    try {
      ...
      // 跳过版权声明、空行等无效行，不计入错误
      if (
        error.message.includes("Invalid row") ||
...
export const FIELD_NOTES = {
  headwords_jyutping:
    '词头和拼音，格式: "词头1:拼音1,词头2:拼音2"，支持多个变体',
