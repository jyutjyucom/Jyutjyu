...
export const DICTIONARY_INFO = {
  ...
  attribution: {
    ...
    "zh-Hant":
      "《實用廣州話分類詞典》，麥耘、譚步雲編，廣東人民出版社，1997年版",
    "yue-Hans":
      "《实用广州话分类词典》，麦耘、谭步云编，广东人民出版社，1997年版",
    "yue-Hant":
      "《實用廣州話分類詞典》，麥耘、譚步雲編，廣東人民出版社，1997年版",
    ...
    if (beforeText) {
      ...
      if (trimmedBefore.length > 0) {
        ...
        if (trimmedBefore.length > replaceLength) {
          ...
          segments.push({
            type: "fixed",
            text: trimmedBefore.slice(0, -replaceLength),
          });
        }
...
export function transformRow(row) {
  ...
  const rawJyutping = row.jyutping
    ...
    : "";
  const jyutpingArray = rawJyutping ? parseJyutpingVariants(rawJyutping) : [];

  // 5. 构建分类路径
  const categories = [row.category_1, row.category_2, row.category_3].filter(
    (c) => c && c.trim(),
  );
  ...
  const entry = {
    ...
    meta: {
      ...
      // 异形词
      headword_variants:
        headwordMarkers.variants.length > 0 ? headwordMarkers.variants : null,

      // 重见标记
      has_cross_reference:
        headwordMarkers.hasCrossReference || crossReferences.length > 0,
