...
export const DICTIONARY_INFO = {
  ...
  description: {
    "zh-Hans":
      "追溯粤语词汇的来源和演变，引用大量古籍文献，展示粤语词汇的历史渊源",
    "zh-Hant":
      "追溯粵語詞彙的來源和演變，引用大量古籍文獻，展示粵語詞彙的歷史淵源",
    "yue-Hans":
      "追溯粤语词汇嘅来源同演变，引用大量古籍文献，展示粤语词汇嘅历史渊源",
    "yue-Hant":
      "追溯粵語詞彙嘅來源同演變，引用大量古籍文獻，展示粵語詞彙嘅歷史淵源",
  },
...
function parseReference(refText) {
  ...
  // 尝试从主文本提取作者和作品
  // 格式: 朝代·作者《作品》 或 作者《作品》
  const authorWorkMatch = mainText.match(
    /^((?:[^·《]+·)?[^《]+)《([^》]+)》[：:]?\s*(.*)$/,
  );
...
export function transformEntry(rows) {
  ...
  const entry = {
    ...
    meta: {
      ...
      // 校对备注
      notes: firstRow.proofreaders_note
        ? parseNote(firstRow.proofreaders_note)
        : null,
...
export function aggregateEntries(entries) {
  ...
  grouped.forEach((group) => {
    if (group.length === 1) {
      ...
      const allHaveVariantNumber = group.every(
        (e) => e.meta.variant_number !== null,
      );
      ...
      if (allHaveVariantNumber) {
        ...
        // 按 variant_number 排序
        group.sort(
          (a, b) => (a.meta.variant_number || 0) - (b.meta.variant_number || 0),
        );
