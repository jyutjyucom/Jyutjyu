...
function parseJyutpingVariants(jyutping) {
  ...
  while ((match = bracketRegex.exec(text)) !== null) {
    ...
    if (beforeText) {
      ...
      const hasComma =
        variantContent.includes(",") || variantContent.includes("，");
      ...
      if (beforeSyllables.length > 0) {
        ...
        const replaceCount = Math.min(
          firstVariantSyllableCount,
          beforeSyllables.length,
        );
...
export const DICTIONARY_INFO = {
  ...
  author: {
    "zh-Hans": "饶秉才、欧阳觉亚、周无忌",
    "zh-Hant": "饒秉才、歐陽覺亞、周無忌",
    "yue-Hans": "饶秉才、欧阳觉亚、周无忌",
    "yue-Hant": "饒秉才、歐陽覺亞、周無忌",
  },
  publisher: {
    "zh-Hans": "广东人民出版社",
    "zh-Hant": "廣東人民出版社",
  ...
  description: {
    "zh-Hans": "系统收录广州话词汇，包含释义、读音与用例。",
    "zh-Hant": "系統收錄廣州話詞彙，包含釋義、讀音與用例。",
    "yue-Hans": "系統收錄廣州話詞彙，包含釋義、讀音與用例。",
    "yue-Hant": "系統收錄廣州話詞彙，包含釋義、讀音與用例。",
  },
  ...
  attribution: {
    ...
    "zh-Hant":
      "《廣州話詞典（第2版）》，饒秉才、歐陽覺亞、周無忌編，廣東人民出版社，2020年版",
    "yue-Hans":
      "《广州话词典（第2版）》，饶秉才、欧阳觉亚、周无忌编，广东人民出版社，2020年版",
    "yue-Hant":
      "《廣州話詞典（第2版）》，饒秉才、歐陽覺亞、周無忌編，廣東人民出版社，2020年版",
...
export function transformRow(row) {
  ...
  const entry = {
    ...
    meta: {
      ...
      // 异形词（从圆括号和方括号中提取）
      headword_variants:
        variantInfo.variants.length > 0 ? variantInfo.variants : undefined,
...
export const FIELD_NOTES = {
  ...
  definition:
    "释义，可能包含多义项（❶❷❸ 或 ①②③）与例句；例句翻译常用〔〕表示。释义中的 〖...〗 会被提取为作者备注，存入 meta.notes",
  page: "词典页码",
