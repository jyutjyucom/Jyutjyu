    ...
    <div class="min-h-screen bg-parchment dark:bg-stone-950">
        ...
        >
            ...
            <div v-else-if="!wordData" class="text-center py-16">
                <h1
                    class="text-2xl font-semibold text-ink dark:text-parchment mb-2"
                >
            ...
            <div v-else class="font-cjk-content">
                ...
                <div v-if="activePronunciationGroup">
                    ...
                    >
                        ...
                        <span class="mx-2 text-graphite/30 dark:text-stone-400"
                            >·</span
                        >
                        {{
                    ...
                    <div class="hidden lg:grid lg:grid-cols-12 gap-8">
                        <aside class="lg:col-span-4 xl:col-span-3">
                            <div
                                class="sticky"
                                :style="{ top: `${appHeaderHeight + 24}px` }"
                            >
                                <nav class="space-y-1">
                                    <button
                        ...
                        <section class="lg:col-span-8 xl:col-span-9">
                            <WordSourcePanel
                                ...
                                :source-label="
                                    selectedDesktopSourceGroup.sourceLabel
                                "
                                :entries="selectedDesktopSourceGroup.entries"
                                :tab-jyutping-list="
                                    activePronunciationGroup.jyutpingList
                                "
                                :collapsible="false"
                                :expanded="true"
                                :active="true"
                    ...
                    <div class="lg:hidden space-y-2">
                        <WordSourcePanel
                            v-for="(
                                source, sourceIdx
                            ) in activePronunciationGroup.sources"
                            :key="`${activePronunciationGroup.id}:${source.id}`"
                ...
                <template v-if="relatedWords.length > 0">
                    <div class="flex items-center gap-3 my-8">
                        <div
                            class="flex-1 h-px bg-kapok/30 dark:bg-kapok/20"
                        ></div>
                        <div
                            class="w-1.5 h-1.5 rounded-full bg-kapok/60 dark:bg-kapok/40"
                        ></div>
                            ...
                            >
                                <div class="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p
                                            class="text-sm text-graphite dark:text-stone-200 mt-2"
                                        >
                                            <span
                                                v-if="
                                                    getEntryJyutpingKey(
                                            ...
                                            <span
                                                ...
                                                >{{
                                                    group.primary.senses[0].definition.slice(
                                                        0,
                                                        40,
                                                    )
                                                }}{{
...
const relatedWords = computed(() => {
    ...
    return backSearchAggregated.value.filter((group) => {
        const display = normalizeComparable(
            group.primary.headword.display || "",
        );
...
const getSourcePriority = (sourceBook: string) => {
    return (
        getSourcePriorityMap.value.get(sourceBook) ?? Number.MAX_SAFE_INTEGER
    );
};
...
const pronunciationGroups = computed<PronunciationGroup[]>(() => {
    ...
    jpMap.forEach((jpEntries, jpKey) => {
        ...
        groups.push({
            ...
            sourcePriority:
                sources[0]?.sourcePriority ?? Number.MAX_SAFE_INTEGER,
        });
    });
...
const selectDesktopSource = (sourceId: string) => {
    const group = activePronunciationGroup.value;
    if (!group || !group.sources.some((source) => source.id === sourceId))
        return;
    selectedSourceByJyutping.value = {
        ...selectedSourceByJyutping.value,
        [group.id]: sourceId,
...
const primaryDictionaryName = computed(() => {
    ...
    const fallbackSourceBook =
        wordData.value?.entries?.[0]?.source_book?.trim();
