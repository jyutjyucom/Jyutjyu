...
export const LOCALE_ROUTE_DEFINITIONS: LocaleRouteDefinition[] = [
  ...
  { code: 'yue-Hans', name: '简体粤文', language: 'yue-Hans', prefix: '/yue-Hans' },
  { code: 'zh-Hant', name: '繁體普通話', language: 'zh-Hant', prefix: '/zh-Hant' },
  { code: 'zh-Hans', name: '简体普通话', language: 'zh-Hans', prefix: '/zh-Hans' },
  { code: 'en', name: 'English', language: 'en', prefix: '/en' }
]

export const NON_DEFAULT_LOCALE_PREFIXES = LOCALE_ROUTE_DEFINITIONS
...
export const cleanHeadwordForPath = (headword: string): string => {
  return String(headword || '').replace(ZERO_WIDTH_CHARACTERS, '').trim()
}
...
export const buildSeoRoutePath = (
  ...
  const normalizedPath = stripLocalePrefix(path)
  const locale = LOCALE_ROUTE_DEFINITIONS.find((item) => item.code === localeCode)
...
export const buildSeoAlternateLinkDefinitions = (
  ...
  LOCALE_ROUTE_DEFINITIONS.forEach((locale) => {
    ...
    const localizedHref = withSiteUrl(siteUrl, buildSeoRoutePath(path, query, locale.code))
  ...
  links.push({
    id: 'i18n-xd',
    hreflang: 'x-default',
    href: withSiteUrl(siteUrl, buildSeoRoutePath(path, query, DEFAULT_LOCALE_CODE))
  })
...
export const buildSitemapUrlEntryXml = (
  ...
  const alternates = LOCALE_ROUTE_DEFINITIONS
    .map((locale) => {
  ...
  return LOCALE_ROUTE_DEFINITIONS
    .map((locale) => {
      const href = withSiteUrl(siteUrl, applyLocalePrefix(path, locale.prefix))
      return [
        '  <url>',
