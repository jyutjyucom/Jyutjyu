...
export const useDictionary = () => {
   ...
   * 获取所有分片词典的配置
   */
  const getChunkedDictionaries = async (): Promise<
    Array<{ id: string; chunk_dir: string }>
  > => {
  ...
  const getRequiredChunks = (query: string, manifest: any): string[] => {
    ...
    const firstAlphaNumericChar =
      queryChars.find((char) => /[a-z0-9]/i.test(char)) || firstChar;
  ...
  const getAllEntries = async (): Promise<DictionaryEntry[]> => {
    ...
    cachePromise = (async () => {
      try {
          ...
          dictionaries.map(async (dict: any) => {
            try {
              ...
              if (dict.chunked) {
                ...
                if (import.meta.dev)
                  console.log(`跳過分片詞典: ${dict.id} (按需加載)`);
                return;
              }
        ...
        if (import.meta.dev)
          console.log(`詞典資料已載入並快取: ${allEntries.length} 筆`);
        return allEntries;
      } catch (error) {
  ...
  const calculateSecondaryScore = (
    ...
    if (entry.source_book === "广州话俗语词典") {
      ...
      entry.source_book === "粵典 (words.hk)" ||
      entry.source_book === "粵典"
    ) {
      score += 4;
    }
  ...
  const searchBasic = async (
    ...
    const startsWithNonChinese = firstVisibleChar
      ? !/[\u3400-\u9fff]/.test(firstVisibleChar)
      : false;
    const needsMixedQueryChunkFallback =
      hasChineseChars && startsWithNonChinese;

    // 获取简繁体转换器并确保已初始化
    const { toSimplified, toTraditional, ensureInitialized } =
      useChineseConverter();
    ...
    // 收集所有结果
    const allResultsWithPriority: Array<{
      entry: DictionaryEntry;
      priority: number;
    ...
    const processEntry = (
      ...
      if (priority > 0) {
        ...
        return {
          priority,
          secondaryScore: calculateSecondaryScore(
            entry,
     ...
     */
    const sortAndPush = (
      results: typeof allResultsWithPriority,
      isComplete: boolean,
    ) => {
      results.sort((a, b) => {
    ...
    try {
      ...
      for (const { chunk_dir } of chunkedDicts) {
        ...
        if (manifest) {
            ...
            if (chunkCache[chunkKey]) {
              ...
              try {
                const response = await fetch(
                  `/dictionaries/${chunk_dir}/${chunk}.json`,
                );
                if (response.ok) {
                  const data = await response.json();
                  if (Array.isArray(data)) {
          ...
          if (searchDefinition && chunk_dir === "cantowords") {
            ...
            if (import.meta.dev)
              console.log(
                `反查模式：載入 ${chunk_dir} 所有分片 (${requiredChunks.length} 個)`,
              );
          } else {
          ...
          if (!searchDefinition && needsMixedQueryChunkFallback) {
            const fallbackChunks = Object.keys(manifest.chunks || {}).filter(
              (chunk) => !processedChunks.has(chunk),
            );
            if (fallbackChunks.length > 0) {
              if (import.meta.dev)
                console.log(
                  `混合查詢補全掃描 ${chunk_dir} 剩餘分片 (${fallbackChunks.length} 個)`,
                );
      ...
      const finalResults = allResultsWithPriority
        .slice(0, limit)
        .map((item) => item.entry);
  ...
  const searchAdvanced = async (
    ...
    if (options.dialect) {
      filteredEntries = entries.filter(
        (e) => e.dialect.name === options.dialect,
      );
    }
    ...
    if (options.source_book) {
      filteredEntries = filteredEntries.filter(
        (e) => e.source_book === options.source_book,
      );
    }
    ...
    if (options.entry_type) {
      filteredEntries = filteredEntries.filter(
        (e) => e.entry_type === options.entry_type,
      );
    }
  ...
  const getDictionaries = async () => {
    try {
      const index = await queryContent("dictionaries/index").findOne();
