...
export const useDictionaryEntry = () => {
  ...
  const getEntryOriginalPhoneticList = (entry: DictionaryEntry): string[] => {
    const original = entry.phonetic?.original as
      | string
      | string[]
      | undefined
      | null;
  ...
  const getEntryFeedbackDescription = (entry: DictionaryEntry): string => {
    ...
    const headerLines: string[] = [
      ...
      entry.headword.normalized !== entry.headword.display
        ...
        : "",
      line(
        "feedback.prefill.jyutping",
        (entry.phonetic.jyutping || []).join(":"),
      ),
      entry.phonetic.original &&
      entry.phonetic.original !== (entry.phonetic.jyutping || []).join(":")
        ? line(
            "feedback.prefill.originalPhonetic",
            String(entry.phonetic.original),
          )
        : "",
      entry.meta?.headword_variants && entry.meta.headword_variants.length > 0
        ? line(
            "feedback.prefill.variantWords",
            entry.meta.headword_variants.join(
              t("feedback.prefill.listSeparator"),
            ),
          )
        : "",
      entry.entry_type
        ? line("feedback.prefill.entryType", getEntryTypeLabel(entry))
        : "",
      "",
    ].filter(Boolean);
        ...
        sense.sub_senses.forEach((sub) => {
          ...
          if (sub.examples && sub.examples.length > 0) {
            sub.examples.forEach((ex) => {
              ...
              if (ex.jyutping)
                senseLines.push(
                  `    ${line("feedback.prefill.jyutping", ex.jyutping)}`,
                );
              if (ex.translation)
                senseLines.push(
                  `    ${line("feedback.prefill.translation", ex.translation)}`,
                );
            });
          }
        });
    ...
    if (entry.meta?.etymology && typeof entry.meta.etymology === "string") {
      headerLines.push(
        line("feedback.prefill.etymology", entry.meta.etymology),
        "",
      );
    }
    ...
    if (entry.refs && entry.refs.length > 0) {
      headerLines.push(
        line(
          "feedback.prefill.seeAlso",
          entry.refs
            .map((r) => r.target)
            .join(t("feedback.prefill.listSeparator")),
        ),
        "",
      );
    }
