...
export const useBrowsePageData = (params: BrowsePageParams) => {
  const manifestState = useState<BrowseManifest | null>(
    "browse-index-manifest",
    () => null,
  );
  ...
  return useAsyncData<BrowseResponse>(
    cacheKey,
    async () => {
      const safeScope = normalizeScope(params.scope.value);
      const safePage = normalizePage(params.page.value);
      ...
      if (manifest) {
        const staticResult = await loadStaticBrowsePage(
          manifest,
          safeScope,
          safePage,
          safePageSize,
